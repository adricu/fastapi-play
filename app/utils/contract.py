...
def get_acr_balance(address, block_identifier="latest"):
...
def get_nft_balance(address, block_identifier="latest"):
...
def get_current_block_number() -> int:  # pragma: no cover
    ...
    return settings.W3.eth.block_number


def get_latest_block_timestamp():  # pragma: no cover
    """Returns the timestamp of the latest block"""
    return settings.W3.eth.get_block("latest").timestamp
